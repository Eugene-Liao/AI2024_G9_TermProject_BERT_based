...
class PostMetadataDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        numerical_data = item[['verified', 'protected', 'followers_count', 'friends_count', 'favorites_count', 'statuses_count']].apply(pd.to_numeric, errors='coerce').fillna(0).values
        numerical_data = torch.tensor(numerical_data, dtype=torch.float)
        
        return user_index, label, embeddings, numerical_data
...
def PostMetadataCustom_collate(batch): 
    ...
    batch_embeddings = torch.stack([item[2] for item in batch])
    batch_numerical_data = torch.stack([item[3] for item in batch])

    return batch_user_indices, batch_labels, batch_embeddings, batch_numerical_data